[![pipeline status](https://gitlab.com/aruw/controls/standard/badges/develop/pipeline.svg)](https://gitlab.com/aruw/controls/standard/-/commits/develop)
...
```
cd my-amazing-project/standard-project
pipenv shell
# Build for hardware
scons build
# Run automated tests
...
1. Make sure you have VSCode opened in the folder `standard` (**not
   `standard-project`**)
...
1. Open the folder `aruw-standard-project` in VSCode. Hit the debug tab on the left side or type
...
`/standard-project/robot-type/robot_type.hpp` and change the macro defined in this file.
...
The below commands require that your working directory is `/standard-project` (where the
